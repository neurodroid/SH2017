...
NEURON {
    ...
    GLOBAL vShift, vShift_inact, maxrate, temp, q10, tadj, vRestNL
    RANGE vShift_inact_local, nonlinear
    RANGE g, gbar
...
PROCEDURE rates(v(millivolt)) {
    ...
    vS = vFix-vShift : note that vShift is subtracted from v, not added to v (unlike trates(v+vshift) in na.mod)
