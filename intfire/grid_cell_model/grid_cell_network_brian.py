...
class BrianGridCellNetwork(GridCellNetwork):
    ...
    def __init__(self, neuronOpts, simulationOpts):
        ...
        self.eqs_e = Equations('''
            ...
            Isyn        = B_GABA*(gi1 - gi2)*(Esyn_i - vm) + gext*(Esyn_e - vm)        : amp
            Iclamp      = -(B_GABA*(gi1 - gi2)*(Esyn_i - Vclamp) + gext*(Esyn_e - Vclamp)) : amp
        ...
        self.net_Nconst = int(self.net_Ne/100.0)
        self.ConstInputE = PoissonGroup(
            N = self.net_Nconst,
            rates=self.no.rates_e_const * hertz,
            clock=self._simulationClock)


        if self.no.NMDA_half < -500.0:
            pre_equ = '''
                      x += w
                      sAMPA += w
                      '''
            ...
            pre_equ = '''
                      x += w * (1.0 - 1.0/(1.0+exp((vm_post-{0})/10.0)))
                      sAMPA += w
                      '''.format(self.no.NMDA_half)
        print(pre_equ)
        self.ConstSynapseE = Synapses(
            self.ConstInputE,
            self.E_pop,
            model = SynapticEquations('''
                stot      = NMDA_amount_ext*0.01*sNMDA + sAMPA                : 1
                dsAMPA/dt = -sAMPA/syn_tau                                    : 1
                dsNMDA/dt = -sNMDA/tau_NMDA_fall + alpha_NMDA * x * (1-sNMDA) : 1
                dx/dt     = -x/tau_NMDA_rise                                  : 1
                w                                                             : 1
                ''',
                NMDA_amount_ext = self.no.NMDA_amount_ext,
                tau_NMDA_fall = self.no.tau_NMDA_fall * ms,
                tau_NMDA_rise = self.no.tau_NMDA_rise * ms,
                alpha_NMDA = self.no.alpha_NMDA * kHz,
                syn_tau = self.no.tau_AMPA_ext * ms),
            pre = pre_equ)
        self.E_pop.gext = self.ConstSynapseE.stot

        self.net = Network(self.E_pop, self.I_pop, self.ConstInputE)
        ...
        for ne in range(self.net_Ne):
            indices = random.sample(range(self.net_Nconst), 1)
            self.ConstSynapseE[indices, ne] = True
            self.ConstSynapseE.w[indices, ne] = self.no.gext_e_const * nS
    ...
    def uniformInhibition(self):
        ...
        self.extraGABA_conn1.connect_random(self.I_pop, self.E_pop, self.no.uni_GABA_density,
                weight=self.B_GABA*g_GABA_mean, seed=self.seed)
    ...
    def uniformExcitation(self):
        ...
        self.uniAMPA_conn.connect_random(self.E_pop, self.E_pop, self.no.uni_AMPA_density,
                weight=g_AMPA_mean, seed=self.seed)
    ...
    def setThetaCurrentStimulation(self):
        ...
        def thetaStimulationFun():
            if self.seed is not None:
                np.random.seed(int(self._simulationClock.t/self._simulationClock.dt))
            global place_flag
            global place_I
    ...
    def setGaussianThetaCurrent(self):
        ...
        def thetaStimulationFun():
            if self.seed is not None:
                np.random.seed(self.seed + int(self._simulationClock.t/self.simulationClock.dt))
            global place_flag
            global place_I
            if self._simulationClock.t < self.no.theta_start_t*msecond:
