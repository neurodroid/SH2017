...
stateMon_sAMPA_e     = RecentStateMonitor(ei_net.ConstSynapseE, 'sAMPA',   duration=options.stateMonDur*ms,   record = [0,], clock=simulationClock)
stateMon_sNMDA_e     = RecentStateMonitor(ei_net.ConstSynapseE, 'sNMDA',   duration=options.stateMonDur*ms,   record = [0,], clock=simulationClock)
...
ei_net.net.add(stateMon_sAMPA_e, stateMon_sNMDA_e)
...
for dump_it in range(options.ndumps):
    ...
    print "Mean E network firing rate:", Fe[:, len(Fe_t)/2:].mean()
    np.savetxt(output_fname + '_mean_firing_rate_e.txt',
               [Fe[:, len(Fe_t)/2:].mean()])

    Fe_path = np.array([
        np.unravel_index(np.reshape(Fe[:, nt], (ei_net.Ne_y, ei_net.Ne_x)).argmax(),
                         (ei_net.Ne_y, ei_net.Ne_x))
        for nt in range(len(Fe_t))])
    ...
    figure(figsize=figSize)
    subplot(211)
    plot(stateMon_sAMPA_e.times, stateMon_sAMPA_e.values)
    xlabel('Time (s)')
    ylabel('sAMPA\_ext')
    subplot(212)
    plot(stateMon_sNMDA_e.times, stateMon_sNMDA_e.values)
    xlabel('Time (s)')
    ylabel('sNMDA\_ext')
    savefig(output_fname + '_sAMPA.pdf')
    ...
    # Plot network path
    figure()
    plot(Fe_path[:,0], Fe_path[:,1])
    axis('equal')
    savefig(output_fname + '_firing_path_e.pdf')
    ...
    axis('equal')
    tight_layout()
    savefig(output_fname + '_firing_snapshot_e.pdf')
    ...
    savefig(output_fname + '_firing_snapshot_i.pdf')
    ...
    outData['Fe_path']                  = Fe_path
    outData['Fe_times']                 = Fe_t
    outData['Fe_snapshot']              = np.reshape(Fe[:, len(Fe_t)/2], (ei_net.Ne_y, ei_net.Ne_x))
    outData['Fi_snapshot']              = np.reshape(Fi[:, len(Fi_t)/2], (ei_net.Ni_y, ei_net.Ni_x))
    outData['Ne_xy']                    = (ei_net.Ne_y, ei_net.Ne_x)
